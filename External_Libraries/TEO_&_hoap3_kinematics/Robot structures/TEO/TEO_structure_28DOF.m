function robot = TEO_structure_28DOF(mode, angle_unit, length_unit)
% TEO_STRUCTURE
% TEO_STRUCTURE(MODE, ANGLE_UNIT, LENGTH_UNIT)

%   theta01 theta02 theta03 theta04 theta05 theta06     RIGHT LEG
%   theta07 theta08 theta09 theta10 theta11 theta12     LEFT LEG
%   theta13 theta14                                     WAIST
%   theta15 theta16 theta17 theta18 theta19 theta20     RIGHT ARM
%   theta21 theta22 theta23 theta24 theta25 theta26     LEFT ARM
%   theta27 theta28                                     NECK

% Generalized variables with their first and second derivatives
q   = generate_symbolic_vector('theta', 28);
...
% Waist
  ...
  waist.joint(2).name = 'Waist Pitch';
  waist.joint(2).id   = 14;
  waist.joint(2).type = 'r';
  waist.joint(2).q    = q(14);
  waist.joint(2).dq   = dq(14);
  waist.joint(2).ddq  = ddq(14);


% Right Arm
  arms.right.name = 'Right Arm';
  arms.right.joint(1).name = 'Shoulder Pitch';
  arms.right.joint(1).id   = 15;
  arms.right.joint(1).type = 'r';
  arms.right.joint(1).q    = q(15);
  arms.right.joint(1).dq   = dq(15);
  arms.right.joint(1).ddq  = ddq(15);

  arms.right.joint(2).name = 'Shoulder Roll';
  arms.right.joint(2).id   = 16;
  arms.right.joint(2).type = 'r';
  arms.right.joint(2).q    = q(16);
  arms.right.joint(2).dq   = dq(16);
  arms.right.joint(2).ddq = ddq(16);

  arms.right.joint(3).name = 'Shoulder Yaw';
  arms.right.joint(3).id   = 17;
  arms.right.joint(3).type = 'r';
  arms.right.joint(3).q    = q(17);
  arms.right.joint(3).dq   = dq(17);
  arms.right.joint(3).ddq  = ddq(17);

  arms.right.joint(4).name = 'Elbow Pitch';
  arms.right.joint(4).id   = 18;
  arms.right.joint(4).type = 'r';
  arms.right.joint(4).q    = q(18);
  arms.right.joint(4).dq   = dq(18);
  arms.right.joint(4).ddq  = ddq(18);

  arms.right.joint(5).name = 'Wrist Yaw';
  arms.right.joint(5).id   = 19;
  arms.right.joint(5).type = 'r';
  arms.right.joint(5).q    = q(19);
  arms.right.joint(5).dq   = dq(19);
  arms.right.joint(5).ddq  = ddq(19);

  arms.right.joint(6).name = 'Wrist Pitch';
  arms.right.joint(6).id   = 20;
  arms.right.joint(6).type = 'r';
  arms.right.joint(6).q    = q(20);
  arms.right.joint(6).dq   = dq(20);
  arms.right.joint(6).ddq  = ddq(20);

% Left Arm
  arms.left.name = 'Left Arm';
  arms.left.joint(1).name = 'Shoulder Pitch';
  arms.left.joint(1).id   = 21;
  arms.left.joint(1).type = 'r';
  arms.left.joint(1).q    = q(21);
  arms.left.joint(1).dq   = dq(21);
  arms.left.joint(1).ddq  = ddq(21);

  arms.left.joint(2).name = 'Shoulder Roll';
  arms.left.joint(2).id   = 22;
  arms.left.joint(2).type = 'r';
  arms.left.joint(2).q    = q(22);
  arms.left.joint(2).dq   = dq(22);
  arms.left.joint(2).ddq  = ddq(22);

  arms.left.joint(3).name = 'Shoulder Yaw';
  arms.left.joint(3).id   = 23;
  arms.left.joint(3).type = 'r';
  arms.left.joint(3).q    = q(23);
  arms.left.joint(3).dq   = dq(23);
  arms.left.joint(3).ddq  = ddq(23);

  arms.left.joint(4).name = 'Elbow Pitch';
  arms.left.joint(4).id   = 24;
  arms.left.joint(4).type = 'r';
  arms.left.joint(4).q    = q(24);
  arms.left.joint(4).dq   = dq(24);
  arms.left.joint(4).ddq  = ddq(24);

  arms.left.joint(5).name = 'Wrist Yaw';
  arms.left.joint(5).id   = 25;
  arms.left.joint(5).type = 'r';
  arms.left.joint(5).q    = q(25);
  arms.left.joint(5).dq   = dq(25);
  arms.left.joint(5).ddq  = ddq(25);

  arms.left.joint(6).name = 'Wrist Pitch';
  arms.left.joint(6).id   = 26;
  arms.left.joint(6).type = 'r';
  arms.left.joint(6).q    = q(26);
  arms.left.joint(6).dq   = dq(26);
  arms.left.joint(6).ddq  = ddq(26);

% Neck
  neck.joint(1).name = 'Neck Yaw';
  neck.joint(1).id   = 27;
  neck.joint(1).type = 'r';
  neck.joint(1).q    = q(27);
  neck.joint(1).dq   = dq(27);
  neck.joint(1).ddq  = ddq(27);

  neck.joint(2).name = 'Neck Pitch';
  neck.joint(2).id   = 28;
  neck.joint(2).type = 'r';
  neck.joint(2).q    = q(14);
  neck.joint(2).dq   = dq(14);
  neck.joint(2).ddq  = ddq(14);
...
if strcmpi(mode, 'symbolic')
  ...
  chest.link_lengths = generate_symbolic_vector('CHEST_LINK', 2);
  ...
  legs.link_lengths(1) = 146;
  legs.link_lengths(2) = 330;
  legs.link_lengths(3) = 300;
  legs.link_lengths(4) = 33;
  legs.link_lengths(5) = 124;

  % Arms
  arms.link_lengths = zeros(3, 1);
  arms.link_lengths(1) = 340;
  arms.link_lengths(2) = 337;
  arms.link_lengths(3) = 210;

  % Chest
  chest.link_lengths = zeros(2, 1);
  chest.link_lengths(1) = 0;
  chest.link_lengths(2) = 245;

  % Waist
  waist.link_lengths = zeros(2, 1);
  waist.link_lengths(1) = 0;
  waist.link_lengths(2) = 287;

  % Torso
  torso.link_lengths = zeros(2, 1);
  torso.link_lengths(1) = 0;
  torso.link_lengths(2) = 60;

  % Head
  head.link_lengths = zeros(2, 1);
  head.link_lengths(1) = 103; % D.E.: Valor NO REAL
  head.link_lengths(2) = 15;  % D.E.: Valor NO REAL
  ...
  waist.joint(1).orientation = ID;
  waist.joint(1).axis = [0, 0, 1];

  waist.joint(2).origin = waist.joint(2).origin;
  waist.joint(2).orientation = [1, 0, 0; 0, 0, 1; 0, -1, 0];
  waist.joint(2).axis = [0, 1, 0];
  ...
  legs.right.joint(5).origin  = legs.right.joint(4).origin + [0; legs.link_lengths(4); -legs.link_lengths(3)];
  ...
  legs.right.foot.limits.x    = [-40, 68]; %D.E.: Verificar
  legs.right.foot.limits.y    = [-41.5, 31.5]; %D.E.: Verificar
  ...
  legs.left.joint(5).origin  = legs.left.joint(4).origin + [0; -legs.link_lengths(4); -legs.link_lengths(3)];
  ...
  arms.right.joint(4).origin  = arms.right.joint(3).origin + [0; 0; -arms.link_lengths(2)];
  arms.right.joint(5).origin  = arms.right.joint(4).origin + [0; 0; -arms.link_lengths(3)];
  arms.right.joint(6).origin  = arms.right.joint(5).origin;

  arms.right.joint(1).orientation  = [1, 0, 0; 0, 0, 1; 0, -1, 0];
  arms.right.joint(2).orientation  = [0, 0, 1; 0, 1, 0; -1, 0, 0];
  arms.right.joint(3).orientation  = [0, 1, 0; -1, 0, 0; 0, 0, 1];
  arms.right.joint(4).orientation  = [1, 0, 0; 0, 0, 1; 0, -1, 0];
  arms.right.joint(5).orientation  = ID;
  arms.right.joint(6).orientation  = [1, 0, 0; 0, 0, 1; 0, -1, 0];

  arms.right.joint(1).axis = [0, 1, 0];
  arms.right.joint(2).axis = [1, 0, 0];
  arms.right.joint(3).axis = [0, 0, 1];
  arms.right.joint(4).axis = [0, 1, 0];
  arms.right.joint(5).axis = [0, 0, 1];
  arms.right.joint(6).axis = [0, 1, 0];
  ...
  arms.left.joint(4).origin  = arms.left.joint(3).origin + [0; 0; -arms.link_lengths(2)];
  arms.left.joint(5).origin  = arms.left.joint(4).origin + [0; 0; -arms.link_lengths(3)];
  arms.left.joint(6).origin  = arms.left.joint(5).origin;
  ...
  % Default length: mm     %
  %%%%%%%%%%%%%%%%%%%%%%%%%%
  % D.E.: Solo se han modificado los angle_limits
  ...
  arms.right.joint(1).angle_limits  = [-70, 45] * DEGREE2RAD;
  ...
  arms.right.joint(2).angle_limits  = [-70, 15] * DEGREE2RAD;
  ...
  arms.right.joint(3).angle_limits  = [-75, 10] * DEGREE2RAD;
  ...
  arms.right.joint(4).angle_limits  = [-100, 10] * DEGREE2RAD;
  ...
  arms.right.joint(6).angle_limits  = [-100, 10] * DEGREE2RAD;
  arms.right.joint(6).torque_limits = [0.28, 0.36] * M2MM^2;
  arms.right.joint(6).relation      = 0;

  % Left Arm
  arms.left.joint(1).angle_limits  = [-70, 45] * DEGREE2RAD;
  ...
  arms.left.joint(3).angle_limits  = [-75, 10] * DEGREE2RAD;
  arms.left.joint(3).torque_limits = [1.5, 3] * M2MM^2;
  arms.left.joint(3).relation      = 209 / DEGREE2RAD;

  arms.left.joint(4).angle_limits  = [-100, 10] * DEGREE2RAD;
  ...
  arms.left.joint(6).angle_limits  = [-100, 10] * DEGREE2RAD;
  ...
  % Hoap arms are rotated by 90� in shoulder joint
  ...
  % Waist
  % This is the link of the TEO body
  ...
  if (strcmpi(length_unit, 'mm') || strcmpi(length_unit, 'millimeters'))
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Humanoid structure generation %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

robot.origin  = origin;
robot.mass    = total_mass;
robot.g0      = g0;
robot.legs    = legs;
robot.arms    = arms;
robot.waist   = waist;
