function [x_traj, dx_traj, ddx_traj] = poly7_viapoints_trajectory(P, Ts)
%POLY7_VIAPOINTS_TRAJECTORY Seventh-order polynomial trajectory interpolation
%   specifying a sequence of desired points (via-points) without indication
%   on the velocity in these points.
%	[X_TRAJ, DX_TRAJ, DDX_TRAJ] = POLY7_VIAPOINTS_TRAJECTORY (P, TS)
%	provides the trajectory seventh-order polynomial interpolant of the n via-points P with
...
%       P  = [P0 P1 P2];
%       [x, dx, ddx] = poly7_viapoints_trajectory (P, 1e-3);
%
%   Note: With a Seventh-order polynomial trajectory with via-points is not possible to
...
%   positions.
%
%   See also POLY3_VIAPOINTS_TRAJECTORY, POLY5_VIAPOINTS_TRAJECTORY, POLY7_TRAJ.
...
[x_traj, dx_traj, ddx_traj] = poly7_segments_trajectory(P, Ts);
