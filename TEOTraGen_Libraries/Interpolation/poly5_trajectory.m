function [x_traj, dx_traj, ddx_traj] = poly5_trajectory(P0, P1, Ts)
%POLY5_TRAJECTORY Fifth-order polynomial trajectory interpolation.
%	[X_TRAJ, DX_TRAJ, DDX_TRAJ] = POLY5_TRAJECTORY (P1, P2, TS)
%	provides the trajectory fifth-order polynomial interpolant of the points P1 and P2 with
...
%       [x, dx, ddx] = poly5_trajectory (P0, P1, 1e-3);
%

%   See also POLY3_TRAJECTORY, POLY7_TRAJECTORY, POLY5_TRAJ.
...
for ii=1:n
    [x(:,ii), dx(:,ii), ddx(:,ii)] = poly5_traj(P0.x(ii), P1.x(ii), tt, P0.dx(ii), P1.dx(ii), P0.ddx(ii), P1.ddx(ii));
end
