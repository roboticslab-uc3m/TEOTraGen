function [x_traj, dx_traj, ddx_traj] = poly7_segments_trajectory(P, Ts)
%POLY7_SEGMENTS_TRAJECTORY Seventh-order polynomial trajectory interpolation for n segments.
%	[X_TRAJ, DX_TRAJ, DDX_TRAJ] = POLY7_SEGMENTS_TRAJECTORY (P, TS)
%	provides the trajectory seventh-order polynomial interpolant of the n segments of points P with
...
%       P  = [P0 P1 P2];
%       [x, dx, ddx] = poly7_segments_trajectory (P, 1e-3);
%
%   See also POLY3_SEGMENTS_TRAJECTORY, POLY5_SEGMENTS_TRAJECTORY, POLY7_TRAJ.
...
for ii=1:num_points-1
    ...
    for kk=1:n
        [x(:,kk), dx(:,kk), ddx(:,kk)] = poly7_traj(P(ii).x(kk), P(ii+1).x(kk), tt, P(ii).dx(kk), P(ii+1).dx(kk), P(ii).ddx(kk), P(ii+1).ddx(kk));
    end
