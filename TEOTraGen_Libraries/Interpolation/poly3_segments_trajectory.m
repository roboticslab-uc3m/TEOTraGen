function [x_traj, dx_traj, ddx_traj] = poly3_segments_trajectory(P, Ts)
%POLY3_SEGMENTS_TRAJECTORY Third-order polynomial trajectory interpolation for n segments.
%	[X_TRAJ, DX_TRAJ, DDX_TRAJ] = POLY3_SEGMENTS_TRAJECTORY (P, TS)
%	provides the trajectory third-order polynomial interpolant of the n segments of points P with
...
%
%   Example:
%       P0 = set_trajectory_condition(0, rand(3,1), rand(3,1), zeros(3,1));
%       P1 = set_trajectory_condition(2, rand(3,1), rand(3,1), zeros(3,1));
%       P2 = set_trajectory_condition(5, rand(3,1), rand(3,1), zeros(3,1));
%       P  = [P0 P1 P2];
%       [x, dx, ddx] = poly3_segments_trajectory (P, 1e-3);
%
%   Note: With a Third-order polynomial trajectory is not possible to
%   determine initial and final accelerations. So the function will ignore
%   them and it will consider accelerations that allow to obtain the desired initial and final
%   velocities.
%
%   See also POLY5_SEGMENTS_TRAJECTORY, POLY7_SEGMENTS_TRAJECTORY, POLY3_TRAJ.
...
for ii=1:num_points-1
    ...
    for kk=1:n
        [x(:,kk), dx(:,kk), ddx(:,kk)] = poly3_traj(P(ii).x(kk), P(ii+1).x(kk), tt, P(ii).dx(kk), P(ii+1).dx(kk));
    end
