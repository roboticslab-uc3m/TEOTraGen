...
%     0; 0.00325683448936741; -0.311486990906165; 0.796421295515307; -0.484850796032492; -0.0354911450764397;...                    % Left Leg
     0.000000008293997; 0.003105707397729; -0.334840114198044; 0.853817845790951; -0.518977721331556; 0.028042790172747;...         % Left Leg
     0; 0;...                                                                                                                       % Waist
    ...
    case 'Double and Simple' % Double Support followed by Simple Support movement
        ...
        switch Leg %Select the support leg for the Simple Support phase
            case 'Right Leg' % Right Leg
                ...
                % trajectory
                trajectory = insert_trajectory(trajectory, TEO_fields, create_trajectory_structure(pose_CoM, Ts, t0), 'CoM');
                trajectory = insert_trajectory(trajectory, TEO_fields, create_trajectory_structure(pose_RF, Ts, t0), 'RF');
                trajectory = insert_trajectory(trajectory, TEO_fields, create_trajectory_structure(pose_LF, Ts, t0), 'LF');
                trajectory = insert_trajectory(trajectory, TEO_fields, create_trajectory_structure(pose_RH, Ts, t0), 'RH');
                trajectory = insert_trajectory(trajectory, TEO_fields, create_trajectory_structure(pose_LH, Ts, t0), 'LH');

                % Generate trajectory for the double support phase
                [trajectory, d_trajectory, dd_trajectory] = move_double_supportX (delta_ds_com, Ts, [t0;T_ds], trajectory, d_trajectory, dd_trajectory,delta.interpola_CoM_DS);

                % Generate trajectory for the simple support phase
                [trajectory, d_trajectory, dd_trajectory] = move_simple_supportX (delta_ss_com, delta_ss_ff, Ts, [T_ds;T_ds+T_ss/2;T], trajectory, d_trajectory, dd_trajectory, 'Simple Support RF', delta.interpola_CoM_SS, delta.interpola_FF_SS); % Support on Right foot
...
%                 [trajectory, d_trajectory, dd_trajectory] = move_simple_support (delta_ss_com, delta_ss_ff, Ts, [T+T_ds;T+T_ds+T_ss/2;2*T], trajectory, d_trajectory, dd_trajectory, 'Simple Support RF',delta.interpola_CoM_SS,delta.interpola_FF_SS); % Support on Left foot
                ...
                % as support
%                 [q, dq, ddq] = inverse_right_ds_ss_TEOX(q0, trajectory, d_trajectory, dd_trajectory, h, TEO);
                [q, dq, ddq] = inverse_ds_ss_right_jacobian_quat(q0, trajectory, h);
...
%                 [trajectory, d_trajectory, dd_trajectory] = moving_arm ('LH',delta_LH,zeros(6,1),zeros(6,1),Ts, [t0 T], trajectory, d_trajectory, dd_trajectory,delta.interpola_LH);
                ...
                % as support
%                 [q, dq, ddq] = inverse_left_ds_ss_TEOX(q0, trajectory, d_trajectory, dd_trajectory, h);
                  [q, dq, ddq] = inverse_ds_ss_left_jacobian_quat(q0, trajectory, h);
        end
