function [q, dq, ddq] = ik_TEO3(q0, traj, d_traj, dd_traj, h, parameters, SETTINGS_TEO, current_field)
%
...
%     end
    ...
    for kk=2:length(traj.time)
        delta_traj.(SETTINGS_TEO.humanoid_fields(jj).name)(:,kk)=traj.(SETTINGS_TEO.humanoid_fields(jj).name)(:,kk)-traj.(SETTINGS_TEO.humanoid_fields(jj).name)(:,kk-1);%+delta_traj.(SETTINGS_TEO.humanoid_fields(jj).name)(:,kk-1);
        delta_d_traj.(SETTINGS_TEO.humanoid_fields(jj).name)(:,kk)=d_traj.(SETTINGS_TEO.humanoid_fields(jj).name)(:,kk)-d_traj.(SETTINGS_TEO.humanoid_fields(jj).name)(:,kk-1);%+delta_d_traj.(SETTINGS_TEO.humanoid_fields(jj).name)(:,kk-1);
        delta_dd_traj.(SETTINGS_TEO.humanoid_fields(jj).name)(:,kk)=dd_traj.(SETTINGS_TEO.humanoid_fields(jj).name)(:,kk)-dd_traj.(SETTINGS_TEO.humanoid_fields(jj).name)(:,kk-1);%+delta_dd_traj.(SETTINGS_TEO.humanoid_fields(jj).name)(:,kk-1);
    end
end
...
%         if strcmp(current_field,'CoM')
            switch traj.SF(jj)
               case 0      % Double Support
                   pose_RF=pose_quat2rpy(real(h.RF_T_w(q(:,jj))));
               ...
               Ta_RF=Tanalytical(pose_RF(4:6));
               Ta_LF=Tanalytical(pose_LF(4:6));
               
               ddq(1:6, jj+1) = invert_kinematics_standard (q(:,jj),...
                   h.RF_J_w, u_R, 1:6, 1:6,Ta_RF);
               ddq(7:12, jj+1) = invert_kinematics_standard (q(:,jj),...
                   h.LF_J_w, u_L, 1:6, 1:6,Ta_LF);          
            ...
            %             end
               ...
               pose_RF_T_w=pose_quat2rpy(real(h.RF_T_w(q(:,jj))));
               pose_w_T_LF=pose_quat2rpy(real(h.w_T_LF(q(:,jj))));
               ...
               Ta_RF_T_w=Tanalytical(pose_RF_T_w(4:6));
               Ta_LF_T_w=Tanalytical(pose_w_T_LF(4:6));
               
               ddq(1:6, jj+1) = invert_kinematics_standard (q(:,jj),...
                   h.RF_J_w, u_R, 1:6, 1:6,Ta_RF_T_w);
               ddq(7:12, jj+1) = invert_kinematics_standard (q(:,jj),...
                   h.w_J_LF, u_L, 1:6, 1:6,Ta_LF_T_w);          
            ...
            %             end
               ...
               pose_LF_T_w=pose_quat2rpy(real(h.LF_T_w(q(:,jj))));
               pose_w_T_RF=pose_quat2rpy(real(h.w_T_RF(q(:,jj))));
               ...
               Ta_LF_T_w=Tanalytical(pose_LF_T_w(4:6));
               Ta_w_T_LF=Tanalytical(pose_w_T_RF(4:6));
               
               ddq(7:12, jj+1) = invert_kinematics_standard (q(:,jj),...
                   h.LF_J_w, u_L, 1:6, 1:6,Ta_LF_T_w);
               ddq(1:6, jj+1) = invert_kinematics_standard (q(:,jj),...
                   h.w_J_RF, u_R, 1:6, 1:6,Ta_w_T_LF);  
...
%                 pose_quat2rpy(h.CoM_T_RH(q(:,jj))));
            ...
            % Right and left arm
            pose_CoM_T_RH=pose_quat2rpy(real(h.CoM_T_RH(q(:,jj))));
            pose_CoM_T_LH=pose_quat2rpy(real(h.CoM_T_LH(q(:,jj))));
            ...
            e_LH(:,jj) = evaluate_error (delta_traj.LH(:,jj)+CoM_p0_LH,...
               ...
               Ta_CoM_T_RH=Tanalytical(pose_CoM_T_RH(4:6));
               Ta_CoM_T_LH=Tanalytical(pose_CoM_T_LH(4:6));
            ...
            ddq(14:18, jj+1) = invert_kinematics_standard (q(:,jj),...
                h.CoM_J_RH, u_RH, 1:3, 1:5,Ta_CoM_T_RH);
            ddq(19:23, jj+1) = invert_kinematics_standard (q(:,jj),...
                h.CoM_J_LH, u_LH, 1:3, 1:5,Ta_CoM_T_LH);          
...
function ddq = invert_kinematics_standard (q_act, J, e, m, n,Ta)
%EN VEZ de h.RF_J_w, le voy a pasar el calculo de Ja
J1 = J(q_act);
Ja=(Ta)'*J1;
ddq = J1(m, n)\e(m);
end
