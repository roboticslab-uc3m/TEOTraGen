function [q, dq, ddq] = inverse_left_ds_ss_TEO(q0, trajectory, d_trajectory, dd_trajectory, h)

%INVERSE_RIGHT_DS_SS_TEO Inverse Differential Kinematics Algorithm with left foot as support for the robot TEO
%   [Q, DQ, DDQ] = 
%   INVERSE_LEFT_DS_SS_TEO(Q0, TRAJECTORY, D_TRAJECTORY, DD_TRAJECTORY, H)
...
% IK constants
    K_p = 100e-3;	% Position Gain --> ORIGINAL: 1e-3
...
for jj=1:L-1
    ...
    switch trajectory.SF(jj)
        case 0      % double support
            if change==1
                RF_p0_w = pose_quat2rpy(h.RF_T_w(q(:,jj)));
                difference_w=trajectory.CoM(:,jj-1);
            end
            e_RF(:,jj) = evaluate_error (trajectory.CoM(:,jj)+RF_p0_w-difference_w, pose_quat2rpy(h.RF_T_w(q(:,jj))));	% Error Right Foot pose
            e_LF(:,jj) = evaluate_error (trajectory.CoM(:,jj)+LF_p0_w, pose_quat2rpy(h.LF_T_w(q(:,jj))));               % Error Left Foot pose
