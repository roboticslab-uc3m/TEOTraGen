function [q, q_dot, p, R, error_p, error_o] = inverse_kinematics_rightALPHA (Ts, T, q, q_dot, Kp, Ko,  xd, xd_dot, Rd, wd, h, SF)
...
for kk = 1:iterations-1
    ...
    %present velocity
    q_dot(1:6,kk) = Jacobian(q(:,kk))\[xd_dot(:,kk) + Kp*error_p(:,kk); wd(:,kk) + Ko*error_o(:,kk)];
    %next q
    q(1:6,kk+1) = q(1:6,kk) + q_dot(1:6,kk)*Ts;
end
